...
public class EmpDeptSalgradeTests
{
    ...
    public void ShouldCountEmployeesPerDepartment()
    {
        ...
        var result = emps.GroupBy(emp => emp.DeptNo).Select(c => new{DeptNo = c.Key, Count = c.Count()}); 
    ...
    public void ShouldReturnEmployeesWithCommission()
    {
        ...
        var result = emps.Where(emp => emp.Comm != null).SelectMany(emp => new[] {new {emp.DeptNo, emp.Comm}}).ToList();
    ...
    public void ShouldCalculateAverageSalaryPerDept()
    {
        ...
        {
            DeptNo = emp.Key, AvgSal = emp.Average(e => e.Sal)
        }).ToList(); 
    ...
    public void ShouldReturnEmployeesEarningMoreThanDeptAverage()
    {
        ...
        var result = emps.Where(emp => emp.Sal > emps.Where(em => emp.DeptNo == em.DeptNo).Average(em => em.Sal))
